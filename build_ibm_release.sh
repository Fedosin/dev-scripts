...
help() {
    echo "Build a release image for IBM CCM and upload it to quay.io"
    echo ""
...
./build_operator_image.sh -u "$USERNAME" -o cluster-cloud-controller-manager-operator -i 97 -t "$TAG"

echo "Building a release image"
./build_release_image.sh -u "$USERNAME" -a "$OC_REGISTRY_AUTH_FILE" \
    -i ibm-cloud-controller-manager=quay.io/openshift/origin-ibm-cloud-controller-manager:"$RELEASE" \
